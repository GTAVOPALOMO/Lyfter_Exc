...
def classification_searcher(classification, p_list):
    for game in p_list:
        if game[HEAD_ESRB].lower() == classification.lower():
            for key,value in game.items():
                print(f"{key}: {value}") 
            print()
        
games_list = csv_reader("csv_test.csv")
esrb = input("Ingrese clasificación ESRB: ")
classification_searcher(esrb,games_list)