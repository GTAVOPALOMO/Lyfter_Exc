...
def games_developed(p_dev, p_list):
    p_dev = p_dev.capitalize()
    print(f"Videojuegos desarrollados por {p_dev}:")
    for game in p_list:
        if game[HEAD_DEV].capitalize() == p_dev:
            print(f"\t- {game[HEAD_NAME].capitalize()} (Clasificación: {game[HEAD_ESRB]}, Género: {game[HEAD_GENRE]})")

        
games_list = csv_reader("csv_test.csv")
developer = input("Ingrese un desarrollador: ")
games_developed(developer, games_list)
